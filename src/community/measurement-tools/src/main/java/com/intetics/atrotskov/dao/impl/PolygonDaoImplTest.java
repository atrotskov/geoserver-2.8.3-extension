...
public class PolygonDaoImplTest implements PolygonDao {

	private Connection<GridCoverage2DReader> conn;
	private CheckerDao checkerDao;

	public PolygonDaoImplTest(Connection<GridCoverage2DReader> conn, CheckerDao checkerDao) {
		this.conn = conn;
	...
	public List<CloudEntity> getValuesByCoord(Coordinate[] coords)
		...
		int listDim = (endPosition.x - startPosition.x) * (endPosition.y - startPosition.y);
		List<CloudEntity> listOfResults = new ArrayList<>(listDim);
		
		GridEnvelope2D ge = new GridEnvelope2D(0, 0, 1, 1);
		Envelope2D pixelEnvelop = geometry.gridToWorld(ge);
		Coordinate point = new Coordinate(pixelEnvelop.getCenterX(), pixelEnvelop.getCenterY());
		GridCoordinates2D gc = new GridCoordinates2D(0, 0);

		for (int j = startPosition.y; j <= endPosition.y; j++) {
			for (int i = startPosition.x; i <= endPosition.x; i++) {
				ge.setLocation(i, j);
				pixelEnvelop = geometry.gridToWorld(ge);
				point.setOrdinate(0, pixelEnvelop.getCenterX());
				point.setOrdinate(1, pixelEnvelop.getCenterY());

				if (polygon.contains(geometryFactory.createPoint(point))) {
					gc.setCoordinateValue(0, i);
					gc.setCoordinateValue(1, j);
					double tempValue = coverage.evaluate(gc, conn.getVals())[0];
					...
					CloudEntity ce = new CloudEntity(value, point, gc);
